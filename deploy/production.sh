...
echo "    * * * * * ${ORIGINAL_PHP_BIN} $PROJECT_DIR/artisan queue:work --max-time=55 --tries=3 >> $PROJECT_DIR/storage/logs/queue-cron.log 2>&1"
echo "    (JANGAN tambahkan --stop-when-empty — itu bikin worker keluar sebelum balasan AI"
echo "    yang tertunda simulasi 'mengetik' sempat tersedia, menambah ~1 menit jeda ekstra)"
echo "    Kedua baris memakai PHP ASLI ($ORIGINAL_PHP_BIN), BUKAN wrapper yang di-heal di"
echo "    atas — routes/console.php SENGAJA hanya memakai Schedule::job(...), yang dispatch"
echo "    ke queue database in-process dan tidak butuh proc_open sama sekali (beda dari"
echo "    Schedule::command(...) yang fork subprocess). Wrapper proc_open di atas hanya"
echo "    untuk proses deploy ini sendiri (Composer/Tinker)."
echo "    PENTING: setelah menyimpan cron di cPanel, verifikasi ulang dengan 'crontab -l'"
echo "    di Terminal — tampilan 'saved' di UI cPanel pernah gagal benar-benar tersimpan"
echo "    (insiden 2026-08-20, baru ketahuan berhari-hari kemudian)."
